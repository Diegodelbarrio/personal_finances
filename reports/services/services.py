...
def get_financial_annual_report(user, year):
    # Ya viene recortado (ej: si es enero, solo trae 1 elemento)
    monthly_data = get_annual_cashflow_summary(user, year)
    
    # Contamos meses con actividad real para promedios justos
    ...
    # Medias anuales basadas solo en meses transcurridos con actividad
    ...
    return {
        "year": year,
        "monthly_data": monthly_data, # Lista corta (ej: 1 mes si estamos en enero)
...
def get_investment_annual_report(user, year):
    ...
    detailed_data = get_investment_detailed_evolution(user, year) # Llamada a la nueva función
